...
export default function cookTimeValidation(cook_time) {
  ...
  if (typeof cook_time === "string" && cook_time.includes(":")) {
    ...
    if (Number.isNaN(hours) || Number.isNaN(mins)) {
      throw new ValidationError("Invalid cook time format. Expected hh:mm");
    }
    if (mins < 0 || mins >= 60) {
      throw new ValidationError("Minutes need to be 0-59");
    }
    if (hours < 0) {
      throw new ValidationError("Hours cannot be less than 0");
    }
